...
class WebcamStreamNode(Node):
    def __init__(self):
        ...
        self.publisher_ = self.create_publisher(CompressedImage, 'webcam_image_2/compressed', 10)  # Change topic name
        # Increase the timer frequency to increase fps
        self.timer = self.create_timer(0.05, self.publish_webcam_image)
        self.cap = cv2.VideoCapture(2)  # 0 represents the default camera (you may need to change this depending on your setup)
